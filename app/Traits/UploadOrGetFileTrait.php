...
trait UploadOrGetFileTrait
{
    ...
    public static function uploadFileToLocalStorage($uploadedFile, $filename = null, $images = null)
    {
        ...
        if ($images == null) {
           $path = $uploadedFile->store($name);
            ...
            $strRand = Str::random(25);
            $name = $name.'/'.$strRand.'.'.$uploadedFile->getClientOriginalExtension();
            Storage::put($name, $images['image_thumb']->__toString());
            $path = '/images/'.$strRand.'.'.$uploadedFile->getClientOriginalExtension();
        }

        return $path;
    }

    public static function uploadFileToS3($uploadedFile, $filename = null, $images = null)
    {
        $name = !is_null($filename) ? $filename : Str::random(25);
        ...
        if ($images == null) {
           $path = $uploadedFile->store($name, 's3');
            ...
            $path = Storage::disk('s3')->putFile($name, $images['image_normal']->__toString());
        }
